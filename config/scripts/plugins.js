...
while (counter<10) {
  ...
  plugin = mote.getSimulation().getCooja().getPlugin("org.contikios.cooja.plugins.LogListener");
  if (plugin != null) {
    plugins++;
  ...
  plugin = mote.getSimulation().getCooja().getPlugin("PowerTracker");
  if (plugin != null) {
    plugins++;
  ...
  plugin = mote.getSimulation().getCooja().getPlugin("org.contikios.cooja.plugins.RadioLogger");
  if (plugin != null) {
    plugins++;
